...
User.findCustomer = result => {
    sql.query(`SELECT * FROM users WHERE userRole = 0`, (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
        return;
  ...
  User.findDealer = result => {
    sql.query(`SELECT * FROM users WHERE userRole = 1`, (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
        return;
